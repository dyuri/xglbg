...
void *pa_fft_thread(void *arg)
{
    ...
    while (t->cont) {
        ...
        if (cfg.mono) {
            ...
            separate_freq_bands(t->fft_buff, t->output_l, smoothing_prec, lcf, hcf, smoothing, sensitivity, t->output_samples);
        } else {
            fftw_execute(t->plan_l);
            fftw_execute(t->plan_r);

            separate_freq_bands(t->fft_buff_l, t->output_l, smoothing_prec / 2, lcf, hcf, smoothing, sensitivity, t->output_samples);
            separate_freq_bands(t->fft_buff_r, t->output_r, smoothing_prec / 2, lcf, hcf, smoothing, sensitivity, t->output_samples);
...
void init_pulse(pa_fft *pa_fft)
{
    ...
    if (!(pa_fft->s = pa_simple_new(NULL, "xglbg", PA_STREAM_RECORD,
                                    pa_fft->dev, "xglbg audio source",
                                    &pa_fft->ss, NULL, &pb,
                                    &pa_fft->error))) {
...
void init_fft(pa_fft *pa_fft)
{
    ...
    pa_fft->plan_l = fftw_plan_dft_r2c_1d(pa_fft->pa_samples, pa_fft->pa_buff_l, pa_fft->output_l, FFTW_MEASURE);
    pa_fft->plan_r = fftw_plan_dft_r2c_1d(pa_fft->pa_samples, pa_fft->pa_buff_r, pa_fft->output_r, FFTW_MEASURE);
}
